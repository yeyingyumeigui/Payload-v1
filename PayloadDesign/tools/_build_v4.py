# -*- coding: utf-8 -*-
"""v4 打包 runner：① 备份 v3 exe（先行、确认成功）② 杀 exe ③ 清 build
④ PyInstaller（envs/default python + PYTHONNET_RUNTIME=netfx，spec 已含 reflector_engine）
⑤ 核验产物。"""
...
LOG = os.path.join(TOOLS, "_build_v4_out.txt")
env = dict(os.environ)
...
# ---- ① 备份 v3 exe（当前 dist 即 v3 最终版 15450314 字节；打包会覆盖，必须先备份）----
dist_exe = os.path.join(TOOLS, "dist", "载荷方案设计器.exe")
bk_dir = os.path.join(BASE, "backup_v3final_20260924")
bk_exe = os.path.join(bk_dir, "载荷方案设计器_v3.exe")
if os.path.exists(dist_exe):
    v3_size = os.path.getsize(dist_exe)
    os.makedirs(bk_dir, exist_ok=True)
    shutil.copy2(dist_exe, bk_exe)
    if os.path.exists(bk_exe) and os.path.getsize(bk_exe) == v3_size:
        log("[备份] v3 exe → %s (%d 字节，核验一致)" % (bk_exe, v3_size))
    else:
        log("[备份] 失败！中止打包（避免丢失 v3）")
        raise SystemExit(1)
else:
...
print("BUILD_V4_RC=%d" % r.returncode)
