# -*- coding: utf-8 -*-
"""v4.1 打包 runner：① 备份 v4 exe（先行、确认成功）② 杀 exe ③ 清 build
...
LOG = os.path.join(TOOLS, "_build_v41_out.txt")
env = dict(os.environ)
...
# ---- ① 备份 v4 exe（当前 dist 即 v4 最终版 15534009 字节；打包会覆盖，必须先备份）----
dist_exe = os.path.join(TOOLS, "dist", "载荷方案设计器.exe")
bk_dir = os.path.join(BASE, "backup_v4final_20260924")
bk_exe = os.path.join(bk_dir, "载荷方案设计器_v4.exe")
if os.path.exists(dist_exe):
    v4_size = os.path.getsize(dist_exe)
    os.makedirs(bk_dir, exist_ok=True)
    shutil.copy2(dist_exe, bk_exe)
    if os.path.exists(bk_exe) and os.path.getsize(bk_exe) == v4_size:
        log("[备份] v4 exe → %s (%d 字节，核验一致)" % (bk_exe, v4_size))
    else:
        log("[备份] 失败！中止打包（避免丢失 v4）")
        raise SystemExit(1)
else:
    log("[备份] dist exe 不存在，跳过备份")
...
print("BUILD_V41_RC=%d" % r.returncode)
