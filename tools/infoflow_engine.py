...
def compute_all(p, kg):
    ...
    rx_flow_sel = flows.get(rx_chain_id)
    if isinstance(rx_flow_sel, dict) and "T_sys" in rx_flow_sel:
        rx = rx_flow_sel
    elif isinstance(rx_flow_sel, dict) and "rx" in rx_flow_sel:
        rx = rx_flow_sel["rx"]
    else:
        rx = propagate_rx(ont.get(rx_chain_id, {}).get("chain", []), p)
    gt = gt_calc(p, ant, rx)
